...
[_SUPER] = LAYOUT_planck_grid(
  RESET, SEL, UPD, CTRL3, ADS, KC_TRNS, KC_TRNS, KC_TRNS, CLAP, MARON, WALK, BL_TOGG,
		BL_DEC, BL_INC, BL_STEP, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, AGREE, AINT, POPCRN, SADMJ,
    ...
    if (record->event.pressed) {
        switch(keycode) {
            ...
            case CLAP:
                ...
                return false;
            case ADS:
                SEND_STRING(SS_DOWN(X_LCTRL)SS_TAP(X_C)SS_UP(X_LCTRL)SS_TAP(X_END)SS_TAP(X_ENTER)"SELECT TOP 1000 * FROM "SS_LCTRL("V")SS_TAP(X_END)SS_DOWN(X_LSHIFT)SS_TAP(X_HOME)SS_UP(X_LSHIFT)SS_TAP(X_F5)SS_TAP(X_BSPACE)SS_TAP(X_HOME)SS_TAP(X_BSPACE)); //SEL TOP 1K FOR ADS
                return false;
            case AGREE:
